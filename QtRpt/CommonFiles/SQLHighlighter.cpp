...
// Revised 2022 Feodor Gvozdev
// SQL syntax highlight plugin for QTextEdit
//

#include "SQLHighlighter.h"

FontHighlightParam SQLHighlighter::getFontColor(int type) {
    FontHighlightParam fh = {.type = Comment,
                             .remark = "comments",
                             .color = "green",
                             .bold = false,
                             .italic = false};
...
void SQLHighlighter::saveSettings(QSettings *settings) {
    settings->beginGroup("SQLHighlighter");
    settings->beginWriteArray("Params");
    for (int i=0; i<lst.size(); ++i) {
        settings->setArrayIndex(i);
        FontHighlightParam fh = lst.at(i);
    ...
    if (settings == 0) {
        ...
        if (listSize > 0) {
            ...
            for (int i=0; i<listSize; ++i) {
                settings->setArrayIndex(i);
                FontHighlightParam fh;
                fh.type = (FontHighlight)settings->value("Type").toInt();
