...
ReportBand *RepScrollArea::m_addBand(QString bandName, BandType type, QMenu *bandMenu, int m_height) {
    QMenu m_bandMenu;
    QList<QAction *> lst = bandMenu->actions();
     for (int i = 0; i < lst.size(); ++i) {
        QAction *button = qobject_cast<QAction *>(lst.at(i));
        if (type == DataGroupHeader) {
            m_bandMenu.addAction(button);
        } else {
            if (button->objectName() != "actGroupProperty") {
                m_bandMenu.addAction(button);
            }
        }
     }
...
void RepScrollArea::correctBandGeom(QWidget *rep) {
    ...
    for (int j=0; j<allReportBand.size(); j++) {
        ...
        widget->move(pageSetting.marginsLeft*m_scale, top_);
...
void RepScrollArea::bandResing(QRect rect) {
    ...
    for (int j=0; j<allReportBand.size(); j++) {
        ...
        widget->setGeometry(pageSetting.marginsLeft*m_scale,
                            top_,
                            (pageSetting.pageWidth*m_scale - pageSetting.marginsLeft*m_scale - pageSetting.marginsRight*m_scale ),
                            widget->baseSize().height()*m_scale);
