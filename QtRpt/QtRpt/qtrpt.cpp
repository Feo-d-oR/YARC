...
void QtRPT::prcessGroupHeader(QPrinter *printer, int &y, bool draw, int pageReport) {
    ...
    if (pageList.at(m_pageReport)->getBand(DataGroupHeader) == 0) {
        ...
        if (listOfPair.size() > 0) {
            ...
            if (pageList.at(pageReport)->getBand(MasterHeader)->showInGroup == 0)
                processMHeader(y,draw);

            int grpNo = 0;
            for (int j = 0; j < listOfPair.size(); ++j) {
                if (listOfPair.at(j).pageReport == pageReport && listOfPair.at(j).paramName == pageList.at(pageReport)->getBand(DataGroupHeader)->groupingField) {
                    bool founded = false;
                    ...
                    if (!founded) { //Start new group
                        ...
                        if (pageList.at(pageReport)->getBand(MasterHeader)->showInGroup == 1)
                            processMHeader(y,draw);
                        processPFooter(draw);
                        processMasterData(printer,y,draw,pageReport);
                        if (pageList.at(pageReport)->getBand(MasterFooter)->showInGroup == 1)
                            processMFooter(printer,y,draw);
                        //---
                        m_recNo = listOfPair.at(j).lnNo;
            ...
            if (pageList.at(pageReport)->getBand(MasterFooter)->showInGroup == 0)
                processMFooter(printer,y,draw);
        }
    }
}
...
void QtRPT::processMFooter(QPrinter *printer, int &y, bool draw) {
    ...
    if (y > ph-mb-mt-pageList.at(m_pageReport)->getBand(MasterFooter)->height)
        newPage(printer, y, draw);
    if (allowPrintPage(draw,curPage)) drawBandRow(pageList.at(m_pageReport)->getBand(MasterFooter), y);
