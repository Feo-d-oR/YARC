...
void ReportBand::setTitleHeight(int h) {
    titleHeight = h;
    QRect r = ui->lblBandType->geometry();
    r.setHeight(h-1);
    ui->lblBandType->setGeometry(r);
}
    ...
    if (obj == ui->conWidget) {        
        if (evt->type() == QEvent::Paint) {
            ...
            for (int i=0; i<allContList.size(); i++) {
                if (!allContList.at(i)->line.p1().isNull()) {
                    QPainter painter(ui->conWidget);
                    painter.setRenderHint(QPainter::Antialiasing,true);
                    if (allContList.at(i)->line.p2().isNull()) {  //Drawing line, the end is Not set                        
                        painter.drawLine(allContList.at(i)->line.p1(), mousePos);                        
                    } else {  //Drawing all lines
                        if (allContList.at(i)->cs != 0 && allContList.at(i)->ce != 0 ) {
                            painter.setPen(QPen(allContList.at(i)->getColorValue(BorderColor),
                                                allContList.at(i)->getBorderWidth(),
                                                QtRPT::getPenStyle(allContList.at(i)->getBorderStyleStr()), Qt::RoundCap));
                            painter.drawLine(allContList.at(i)->cs->pos()+QPoint(3,3), allContList.at(i)->ce->pos()+QPoint(3,3));
                            allContList.at(i)->drawArrow(&painter);
                        }
                    }
                }
            }
...
void ReportBand::newFeildTreeItem(QObject *widget) {
    ...
    if (container != 0) {
        ...
        if (container->getType() == Text || container->getType() == TextImage)
